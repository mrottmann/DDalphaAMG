...
void selfcoupling_LU_doublet_decomposition_PRECISION( const config_PRECISION output, config_double input ) {
  ...
  for ( n=0; n<2; n++ ) {
    ...
    for ( j=0; j<6; j++ ) {
      L[j+6][j+6] = (complex_PRECISION) *in_pt; in_pt++;
    }
    
    in_pt = input+offset[4*n+2];
    for ( j=0; j<5; j++ ) {
      for ( i=j+1; i<6; i++ ) {
        L[j+6][i+6] = L[j][i] = (complex_PRECISION) *in_pt;
        L[i+6][j+6] = L[i][j] = (complex_PRECISION) conj_double(*in_pt); in_pt++;
      }
    }
    ...
    for ( j=0; j<6; j++ ) {
      for ( i=0; i<6; i++ ) {
        L[j+6][i] = L[j][i+6] = _COMPLEX_PRECISION_ZERO;
      }
    }
    for ( j=0; j<6; j++ ) {
      L[j+6][j] = L[j][j+6] = (complex_PRECISION) *in_pt; in_pt++;
    }
...
*********************************************************************************/
  ...
  if( g.n_flavours == 2)
    for ( n=0; n<2; n++ ) {
      complex_PRECISION z[12];
      LU+=66; // moving to U
      // z = U x
      for ( i=11; i>=0; i-- ) { //row
        z[i] = 0;
        for ( j=i+1; j<12; j++ ) { //column
          z[i] += *LU *x[j]; LU++;
        }
        z[i] += *LU *x[i]; LU++;
      }    
...
*********************************************************************************/
  ...
  if( g.n_flavours == 2) {
    ...
    PRECISION *sc_pt = op->clover_doublet_vectorized + 288*(start/24);
    ...
    if ( g.n_flavours == 2 &&
      ...
      apply_doublet_coupling_PRECISION( x, y, epsbar_term, start-end );
#endif
#else 
    config_PRECISION sc = op->clover_doublet + (start/24)*288;
...
void diag_ee_inv_PRECISION( vector_PRECISION y, vector_PRECISION x, operator_PRECISION_struct *op,
  ...
  if( g.n_flavours == 2) {
    ...
    config_PRECISION sc = op->clover_doublet;
    // diagonal blocks applied to the even sites
    for ( i=0; i<n1; i++ ) {
...
*********************************************************************************/
  ...
  if( g.n_flavours == 2) {
    ...
    config_PRECISION sc = op->clover_doublet + n1*288;
    x += n1*24; y += n1*24;
    // diagonal blocks applied to the even sites
    for ( i=0; i<n2; i++ ) {
      LU_multiply_PRECISION( y, x, sc );
...
*********************************************************************************/ 
  ...
  sc_in = in->clover; tm_term_in = in->tm_term;
    
  // re-order clover term (i.e., self coupling)
  MALLOC( op->clover_doublet, complex_PRECISION, lu_doublet_dec_size*n );
  Aee = op->clover_doublet;
  Aoo = op->clover_doublet + op->num_even_sites*lu_doublet_dec_size;
#ifdef OPTIMIZED_SELF_COUPLING_PRECISION
...
void oddeven_free_PRECISION( level_struct *l ) {
  ...
  FREE( l->oe_op_PRECISION.clover_doublet, complex_PRECISION, 288*n );
#else
...
#else  //we don't have the LU decomposition here, for debugging only  
...
#endif
#else
      PRECISION *clover_vectorized = s->op.clover_oo_inv_vectorized + (start/24-block_num*n1)*144;
      for ( i=0; i<n2; i++ ) {
...
void block_oddeven_PRECISION_test( level_struct *l, struct Thread *threading ) {
  ...
  diff = process_norm_PRECISION( b3, 0, vs, l, no_threading ) / process_norm_PRECISION( b2, 0, vs, l, no_threading );
